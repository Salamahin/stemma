...
test("v2 modals screenshots (mobile)", async ({ page }) => {
  ...
  await page.screenshot({ path: "test-results/v2-mobile-m08-person-sheet.png" });
  ...
  if (await shareBtn.isVisible({ timeout: 1_500 }).catch(() => false)) {
    ...
    await page.waitForTimeout(300);
    await page.screenshot({ path: "test-results/v2-mobile-m09-share-empty.png" });

    await page.getByTestId("v2-share-email").fill("guest@example.com");
    await page.waitForTimeout(200);
    await page.screenshot({ path: "test-results/v2-mobile-m10-share-filled.png" });

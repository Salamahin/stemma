...
object stemma {
  ...
  val basic: URLayer[REPO, STEMMA] = (for {
    repo    <- ZIO.environment[REPO].map(_.get)
    service = new StemmaServiceImpl(repo.repo)

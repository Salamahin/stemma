...
export class MutationActions {
    ...
    onGhostClick(
        ...
        if (kind === "child" && focused.kind === "family") {
            this.createPersonInFamily(focused.id, "child", this.t("addChild"), pins.person);
            return;
        }

        if (kind === "child" && focused.kind === "person" && existingFamilyId) {
            this.createPersonInFamily(existingFamilyId, "child", this.t("addChild"), pins.person);
            return;
        }
...
const BRANCH_FROM_PERSON: Record<GhostKind, { titleKey: string; pending: FamilyRole; promote: FamilyRole }> = {
    spouse: { titleKey: "addSpouse", pending: "parent", promote: "parent" },
    parent: { titleKey: "addParent", pending: "child", promote: "parent" },
    child: { titleKey: "addChild", pending: "parent", promote: "child" },
};

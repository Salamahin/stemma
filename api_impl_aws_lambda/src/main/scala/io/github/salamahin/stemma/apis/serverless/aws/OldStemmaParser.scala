...
import io.github.salamahin.stemma.service.{ConfiguredStemmaService, StorageService}
...
//  }
  ...
  def populate(s: StorageService, userId: Long, stemmaId: Long, f: F, oldToNewId: Map[String, Long]) = {
    val parents = f
      .parents
      .map(pid =>
        oldToNewId
  ...
  override def run: ZIO[Any with ZIOAppArgs with Scope, Any, Any] = {
    (for {
      ...
      stemmaId <- s.createStemma(1, "Общий")
      _ <- ZIO.foldLeft(families)(Map.empty[String, Long]) {
            case (oldToNew, f) => populate(s, 1, stemmaId, f, oldToNew)
          }

      stemma <- s.stemma(1, stemmaId)
      _      = println(stemma)
    } yield ())
      .provide(ZLayer.succeed(new ConfiguredStemmaService))
      .foldCause(
